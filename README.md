...
**Requires firmware patch 5 or later** (the [NW-Device-Specification](https://github.com/NorthernWidget/NW-Device-Specification) Schema 1 register map, on Project-Apis `master`) on a board provisioned with [NW-Provision](https://github.com/NorthernWidget/NW-Provision). The default I2C address is `0x41`. Boards running the v0.1.x firmware (address `0x50`) are refused by `begin()`; reflash and provision them, or use Apis_Library v0.1.0. Patch 3 runs the accelerometer at 10 Hz, serves its own temperature beside the axes, and takes the Hall-effect zero (started by a momentary magnet, at boot or later) as an average of samples until it settles, with the temperature at the zero stored beside the offsets. Every row therefore ends with `AccelT [C]`, the chip's relative temperature digit, so that a per-unit drift correction can be fitted from logged data later; `getAccelTemperature()` and `getZeroTemperature()` return the two digits. From patch 5 the unit keeps a record of its zeros: the current one and the two before it on Page 1, with a count of zeros stored since manufacture that every reading carries. `getZeroGeneration()` returns that count as read with the last orientation reading, `zeroChanged()` is true when it differs from the one seen at the reading before (or at `begin()`), and `dumpZeros(Serial)` prints the record, newest first, one line per zero as `generation,X,Y,Z,T`. `printStatus(out)` prints one status line for a logger's status file (name, serial, hardware version, firmware patch and build commit from Page 0, this library's `APIS_LIBRARY_VERSION` and `APIS_LIBRARY_COMMIT`, the last report as code and note, Pages 0 to 2 in hex), for the logger to write with its timestamp whenever `reportKind()` is not zero. The two commits come from the NW-Build wrapper and are blank in an IDE build. Pages renumbered 2026-09-23 (spec 4c3b18d): calibration is Page 1 at 0x20, data Page 2 at 0x40.
